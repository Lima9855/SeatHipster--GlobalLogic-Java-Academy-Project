...
import com.globallogic.seatreservation.web.rest.errors.EntityNotFoundException;
...
import static com.globallogic.seatreservation.domain.enumeration.ReservationStatus.*;
...
public class SeatReservedService {
    ...
    public SeatReserved save(SeatReserved seatReserved) {
        ...
        List<SeatReserved> currentReservations = seatReservedRepository.findByUserAndBetweenDatesAndWithStatus(seatReserved.getFromDate(), seatReserved.getToDate(), EnumSet.of(ACTIVE, PENDING));
        if (!currentReservations.isEmpty()) {
            seatReserved.setReservationStatus(PENDING);
        } else {
            seatReserved.setReservationStatus(ACTIVE);
        }
    ...
    public List<SeatReserved> findByReservedSeatsInGivenPeriodOfTime(ZonedDateTime fromDate, ZonedDateTime toDate, List<Long> seatIds) {
        ...
        if (!seatReservedDateValidation(fromDate, toDate)) {
            ...
            return seatReservedRepository.findByReservedSeatsInGivenPeriodOfTime(fromDate, toDate, seatIds, EnumSet.of(ACTIVE, PENDING));
    }
    ...
    public Page<SeatReserved> findPending(Pageable pageable) {
        ...
        User user = userRepository.findOneByLogin(SecurityUtils.getCurrentUserLogin().orElseThrow(() -> new UserNotFoundException("Current user login not found"))).orElseThrow(() -> new UserNotFoundException("User could not be found"));
        if (user.getAuthorities().stream().anyMatch(auth -> "ROLE_ADMIN".equals(auth.getName()))) {
            return seatReservedRepository.findAllByReservationStatus(pageable, PENDING);
        }
    ...
    public void cancelReservation(Long id) {
        SeatReserved seatReserved = changeReservationStatus(id, CANCELLED);
    ...
    public void approveReservation(Long id) {
        SeatReserved seatReserved = changeReservationStatus(id, ACTIVE);
