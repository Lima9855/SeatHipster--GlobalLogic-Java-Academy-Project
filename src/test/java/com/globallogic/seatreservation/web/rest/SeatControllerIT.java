...
import com.globallogic.seatreservation.domain.enumeration.SeatStatus;
...
/**
 * Integration tests for the {@link SeatController} REST controller.
 */
@IntegrationTest
...
class SeatControllerIT {
    ...
    private static final SeatStatus DEFAULT_STATUS = SeatStatus.FREE;
    private static final SeatStatus UPDATED_STATUS = SeatStatus.UNAVAILABLE;
    ...
    public static Seat createEntity(EntityManager em) {
        Seat seat = new Seat().seatNumber(DEFAULT_SEAT_NUMBER).name(DEFAULT_NAME).status(DEFAULT_STATUS);
        return seat;
    }
    ...
    public static Seat createUpdatedEntity(EntityManager em) {
        Seat seat = new Seat().seatNumber(UPDATED_SEAT_NUMBER).name(UPDATED_NAME).status(UPDATED_STATUS);
        return seat;
    }
    ...
    void putNewSeat() throws Exception {
        ...
        updatedSeat.seatNumber(UPDATED_SEAT_NUMBER).name(UPDATED_NAME).status(UPDATED_STATUS);
    ...
    void partialUpdateSeatWithPatch() throws Exception {
        ...
        partialUpdatedSeat.seatNumber(UPDATED_SEAT_NUMBER).name(UPDATED_NAME).status(UPDATED_STATUS);
